...
app.post('/api/register', async (req, res) => {
    const { name, email, phone, password, user_role, avatar } = req.body;
    ...
    if (useMySQL) {
        try {
            ...
            await db.query(
                `INSERT INTO users (name, email, phone, password, user_role, avatar, alerts_enabled) 
                 VALUES (?, ?, ?, ?, ?, ?, ?)`,
                [name.trim(), (email || '').trim(), phone.trim(), hashedPassword, role,
                avatar || 'assets/user_avatar.png', true]
            );

            const token = generateToken({ name: name.trim(), user_role: role });
            return res.status(201).json({
                message: 'Registration successful',
                user: { name: name.trim(), email: (email || '').trim(), phone: phone.trim(), user_role: role, avatar: avatar || 'assets/user_avatar.png' },
                token
            });
        } catch (err) {
...
app.get('/api/user/profile', authMiddleware, async (req, res) => {
    ...
    if (useMySQL) {
        try {
            const [rows] = await db.query('SELECT name, email, phone, user_role, avatar, alerts_enabled FROM users WHERE name = ?', [req.user.name]);
            if (rows.length > 0) user = rows[0];
        } catch (err) {
            return res.status(500).json({ error: `Database error: ${err.message}` });
        }
...
app.post('/api/user/update', authMiddleware, async (req, res) => {
    const { name, email, phone, alerts_enabled, avatar, user_role, current_password, new_password } = req.body;
    ...
    try {
        ...
        if (useMySQL) {
            await db.query(
                `UPDATE users SET name = ?, email = ?, phone = ?, user_role = ?, alerts_enabled = ?, avatar = ?, password = ? 
                 WHERE name = ?`,
                [
                    name || oldName,
                    email || '',
