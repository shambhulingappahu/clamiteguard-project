...
function onEachCropFeature(feature, layer) {
  ...
  Yield: ${Math.round(yieldVal).toLocaleString()} units<br>
  Area: ${Math.round(areaVal).toLocaleString()} hectares<br>
  Top Crop: ${topCrop}
  `;
...
legendControl.onAdd = function () {
  ...
  div.innerHTML = `
    <div style="font-weight:700; margin-bottom:6px;">Crop Yield (Units)</div>
...
function renderInsights() {
    ...
    document.getElementById('summaryTotalYield').textContent = Math.round(totalYield).toLocaleString() + ' units';
    document.getElementById('summaryTopCrop').textContent = topCrop;
    ...
    if (topListWrapper) {
        ...
        if (sorted.length === 0) {
            topListWrapper.innerHTML = '<li class="list-group-item bg-transparent text-muted small">No data for selection.</li>';
        } else {
...
function renderMajorCrops() {
    ...
    if (sortedCrops.length === 0) {
        container.innerHTML = '<div class="col-12"><p class="text-muted">No crop data available to display.</p></div>';
        return;
    }

    container.innerHTML = sortedCrops.map(([cropName, data]) => {
        // Find top 2 districts for this crop
        const topDists = data.districts.sort((a,b)=>b.y - a.y).slice(0,2).map(d=>d.dist).join(', ');
        ...
        <div class="col-md-6 col-lg-4">
            <div class="card h-100 shadow-sm border-0 feature-card crop-clickable-card"
                ...
                data-crop="${cropName}" data-dists="${topDists}" data-yield="${data.yield}">
                <div class="card-body">
                    <div class="d-flex align-items-center mb-3">
                        <div class="fs-1 me-3">${info.icon}</div>
                        <h5 class="card-title fw-bold text-success mb-0">${cropName}</h5>
                    </div>
                    <p class="card-text small text-muted mb-1"><b>Total Yield:</b> ${Math.round(data.yield).toLocaleString()} units</p>
                    <p class="card-text small text-muted mb-0"><b>Top Districts:</b> ${topDists || 'N/A'}</p>
                </div>
            </div>
        </div>
        `;
    ...
    document.querySelectorAll('.crop-clickable-card').forEach(card => {
        card.addEventListener('click', function() {
            const cName = this.getAttribute('data-crop');
            const cDists = this.getAttribute('data-dists');
            
            const info = cropInfoMap[cName.toLowerCase()] || {icon:'🌱', water:'Unknown', maturity:'Unknown', tip:'Not available.'};
            
            document.getElementById('modalCropTitle').textContent = cName;
            document.getElementById('modalCropIcon').textContent = info.icon;
            document.getElementById('modalCropDesc').textContent = `Major crop in Karnataka.`;
            document.getElementById('modalCropDistricts').textContent = cDists || 'Various';
            document.getElementById('modalCropWater').textContent = info.water;
            document.getElementById('modalCropSeason').textContent = 'Varies (Check map filter)';
            document.getElementById('modalCropMaturity').textContent = info.maturity;
            document.getElementById('modalCropTip').textContent = info.tip;
        });
    });
}
