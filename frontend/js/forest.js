    ...
    topForestDistrictsList.innerHTML = topDistricts
      .map((item, idx) => `
        <li class="list-group-item bg-transparent d-flex justify-content-between align-items-center small">
          <span>${idx + 1}. ${item.district}</span>
          <span class="badge bg-success-subtle text-success border border-success-subtle">${item.area} sq km</span>
        </li>
      `)
      .join('');
  }
...
function onEachForestFeature(feature, layer) {
  ...
  const areaText = typeof area === 'number' ? `${area} sq km` : 'Data unavailable';

  layer.bindTooltip(`<strong>${districtName}</strong><br>Forest Area: ${areaText}`, {
    sticky: true
  });
...
legendControl.onAdd = function () {
  ...
  div.innerHTML = `
    <div style="font-weight:700; margin-bottom:6px;">Forest Density</div>
    <div><i style="background:#00441b"></i> 3000+ sq km</div>
...
function populateForestModal(forestId) {
  ...
  document.getElementById('modalForestTitle').textContent = data.title;
  document.getElementById('modalForestImg').src = data.img;
  document.getElementById('modalForestDesc').textContent = data.description;
  document.getElementById('modalForestDistricts').textContent = data.districts;
  document.getElementById('modalForestStatus').textContent = data.status;
  document.getElementById('modalForestType').textContent = data.type;
  document.getElementById('modalForestArea').textContent = data.area;
  document.getElementById('modalForestWildlife').textContent = data.wildlife;
  document.getElementById('modalForestConservation').textContent = data.conservation;
}
