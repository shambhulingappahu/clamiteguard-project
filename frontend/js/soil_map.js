...
document.addEventListener('DOMContentLoaded', () => {
    ...
    function onEachFeature(feature, layer) {
        ...
        layer.bindTooltip(`<strong>${resolvedDistrict}</strong><br>Soil: ${soilType || 'TBD'}`, {
            sticky: true,
        ...
        .then(data => {
            ...
            if (legend) {
                ...
                sortedSoils.forEach(s => {
                    const label = s.split('-').map(word => word.charAt(0).toUpperCase() + word.slice(1)).join(' ');
                    legendHtml += `
    ...
    document.querySelectorAll('.clickable-card').forEach(card => {
        ...
        card.addEventListener('click', () => {
            ...
            if (data) {
                document.getElementById('modalSoilTitle').textContent = data.title;
                document.getElementById('modalSoilImg').src = data.img;
                document.getElementById('modalSoilDesc').textContent = data.desc;
                document.getElementById('modalSoilPh').textContent = data.ph;
                document.getElementById('modalSoilTexture').textContent = data.texture;
                document.getElementById('modalSoilWater').textContent = data.water;
                document.getElementById('modalSoilDrainage').textContent = data.drainage || '-';
                document.getElementById('modalSoilOrganicCarbon').textContent = data.organic_carbon || '-';
                document.getElementById('modalSoilRegions').textContent = data.top_regions || '-';
                document.getElementById('modalSoilCrops').textContent = data.best_crops || '-';
                document.getElementById('modalSoilTip').textContent = data.management_tip || 'No specific tip available.';
